...
public class ges_activiites extends JFrame implements ActionListener {
	...
	JTextField txtnomact,txtnv,txtequip,txtrec;
	...
	{ 
     ...
     JLabel lbnomact=new JLabel("Nom d'Activité");
     JLabel lbequip=new JLabel("Equipe");
    ...
    // modif=new JButton("Modifier");
     ...
     fer=new JButton("Terminez");  fer.setIcon(new ImageIcon("C:\\Users\\mohakalil\\Desktop\\img\\fer.gif"));
  ...
  //suppression
		...
		String query="DELETE FROM activites WHERE Niveau='"+n+"'";
		...
		if(e.getSource()==btModif)
		{
			...
			String query="UPDATE  activites SET nomAct='"+p+"', equipet='"+pr+"' where Niveau="+n+"";
   ...
   String query="SELECT * FROM activites  WHERE Niveau="+n+"";
		...
		try {
			...
			if(rst.next())
			{   txtnv.setText(rst.getString("Niveau"));
				txtnomact.setText(rst.getString("nomAct"));
				txtequip.setText(rst.getString("equipet"));
